...
- `data/planning/card_ops/01_wb_card_create_from_html.md` - создание
  WB-карточки из owner-approved HTML / Layer 3 паспорта.
- `data/planning/card_ops/02_seller_sku_update_ozon_wb.md` - смена
  артикулов продавца Ozon/WB на внутренний артикул.
...
Для WB-фото ведомственных товаров действует постоянное уточненное правило
владельца от 2026-07-19: ретушь или водяной знак обязательны только для
видимых геральдических ведомственных знаков и гербов. Обычные текстовые
надписи ведомств (`ФСБ`, `ПОЛИЦИЯ`, `ОМОН` и аналогичные) сами по себе под это
правило не подпадают. Фото изделия только с надписью, нейтральные сервисные
слайды и фото вариантов ношения без геральдического знака можно переносить
между площадками без водяного знака. Если на изображении есть герб или
геральдическая ведомственная эмблема, WB media update остается заблокированным
до готовности защищенной версии; текстовые и атрибутные изменения карточки
этим не блокируются.
...
- для owner-approved карточек Vital Shevron правило владельца от 2026-06-28:
  если владелец явно указывает, что `артикул тоже под замену`, seller SKU
  должен быть заменен на внутренний артикул вместе с применением карточки или
  отдельным seller SKU update шагом. Это не отменяет safety-цепочку и не
  разрешает массовую замену артикулов без exact package, drift-check и verify.
...
Для карточек товаров Vital Shevron подтверждение устроено так: если владельцу
отправлен HTML-пакет по конкретной позиции, где явно показаны все изменения и
опасные действия (`before -> after`, Ozon/WB, seller SKU, создание карточки,
фото, атрибуты, хештеги/теги, группировка), и владелец после просмотра пишет
`применяй`, это является полным approval для всех действий, явно показанных в
этом HTML. Агент не должен повторно спрашивать подтверждение по тем же
действиям. Остановиться перед apply нужно только при drift, недостающем
обязательном поле, новом риске, действии, которого не было в HTML, или
расхождении apply-пакета с просмотренным HTML.
