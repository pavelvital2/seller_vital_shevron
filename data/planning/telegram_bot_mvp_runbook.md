...
С 2026-07-29 добавлена отдельная кнопка `Акции от минимальной цены`: скидка
для товаров вне подходящих акций вводится вручную, а при пропуске используется
`50%`; расчет и apply выполняются раздельными задачами Job Worker.
...
- Не отправлять CLI-smoke с `actor=codex` и без Telegram update metadata в
  notifier-enabled systemd Worker как проверку доставки: сама read-only job
  выполнится, но notifier вернет `missing_telegram_update`, а one-shot service
  завершится с кодом `2`. Для проверки вычислительного контура допустим
  CLI `jobs run`; end-to-end проверку systemd Worker + notifier запускать
  реальной Telegram-командой. После диагностического запуска без маршрута
  доставки очистить failed-state, запустить Worker на пустой очереди и
  подтвердить `status=0/SUCCESS`, timer `active/waiting`.
...
## Ozon/WB `Цены и маржа`

Кнопки `Ozon -> Цены и маржа Ozon` и `Wildberries -> Цены и маржа WB`
запускают одинаковый dry-run диалог:
...
4. постановка `ozon-pricing-margin` или `wb-pricing-margin` в Job Worker;
5. Telegram summary и Excel с расходами, ценовой сеткой и товарами.

В чат отдельно выводятся расходы площадки на проданный товар/комплект и на
одно физическое изделие. Цена в кабинете не меняется; callback apply в первой
версии отсутствует. Оба отчета включают срезы 15/30 дней, stock gate и
action gate.

## Маршрут автоматических отчетов

Scheduled Job Worker отчеты владельца должны отправляться отдельным ботом
`Vital Shevron Manager` в owner chat `867144591` без Telegram topic/thread.
Технические Codex topics не являются маршрутом автоматической операционной
отчетности. Значение можно переопределить только явным deployment env.
