...
- Для будущей WB-карточки целевой порядок тот же. На слайдах 1–3 виден
  военный эмблемный знак, поэтому WB media upload заблокирован до защищённых
  версий с watermark VitalEmb на области знака. Нейтральные слайды 4–5 можно
  переносить без изменения. Это не блокирует паспорт, WB create, текст и
  характеристики.
...
- Layer 3 не создавался, marketplace/LK/API write, WB create и dry-run apply
  не выполнялись. Карточка ожидает owner review.
