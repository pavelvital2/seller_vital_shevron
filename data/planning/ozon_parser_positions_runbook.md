...
- Для ценового SEO-среза дополнительно использовать свежий Ozon product info
  snapshot из Seller API: seller `price`, `old_price`, `min_price`, FBO
  комиссии и остатки.
...
- `run_id`, `mtime`, количество уникальных запросов и диапазон
  `collected_at_utc`;
...
- ценовой срез:
  - parser `price` как витринная цена покупателя в публичной выдаче;
  - seller `price`/`old_price`/`min_price` из Ozon API;
  - медиана/минимум/максимум цены top-10/top-30 по запросу;
  - разница parser price и seller API price по нашим товарам;
- ограничения анализа.
...
- Parser `price` не равен цене продавца из Ozon API/ЛК. Это цена, которую
  parser увидел в публичной выдаче для покупателя. Если она ниже seller price,
  нельзя автоматически считать разницу потерей: нужно сверять компенсации,
  баллы, скидки Ozon и взаиморасчеты по финансовым отчетам.
