...
Подтверждено 2026-07-27 для покупательского вопроса с отдельным сообщением-
фотографией:

- Ozon может вернуть самым новым непрочитанным сообщением только Markdown-
  ссылку на фото, а сам вопрос находится в непосредственно предыдущем
  непрочитанном сообщении того же `Customer`;
- в этом случае нельзя создавать `manual_chat_review` только из-за отсутствия
  текста в последнем сообщении;
- классификацию и черновик строить по предыдущему тексту, в owner-review
  показывать и вопрос, и фото, но создавать одну action-строку с
  `message_id/from_message_id` самого нового сообщения;
- после approval ответ на самое новое сообщение должен закрыть весь
  непрочитанный хвост чата; verify выполняется по истории.
...
Фразы покупателя `на заказ делаются`, `под заказ`, `изготовления нету` и `не
печатаете` относятся к подтвержденному правилу Vital Shevron: индивидуальное
изготовление недоступно, покупателю предлагаются только готовые варианты из
магазина. Черновик нужно адаптировать к точному вопросу, а не оставлять такой
диалог в `manual_chat_review`.
...
8. Если после ответа продавца покупатель присылает короткое закрывающее
   ...
   стало `is_read=true`.
9. На 2026-07-28 подтверждено, что `/v2/chat/read` может вернуть
   `403 Premium Plus` и для сервисного/площадочного чата. После owner approval
   применяется тот же точечный LK/CDP fallback. Успех фиксируется только если
   `/v3/chat/history` подтверждает `is_read=true` для точного
   `from_message_id`; простое открытие страницы успехом не считается.

## Состояние обработанного хвоста

### Проверенный apply 2026-07-18

- В составе `ozon_inbox_20260718T0908_final_review_apply` отправлено `5`
  согласованных сообщений и отмечено прочитанными `12` согласованных чатов и
  уведомлений.
- Свежий verify `ozon_inbox_20260718T1015_verify` вернул `0` действий
  Messenger. Все согласованные строки исчезли из очереди.
- Связанные исходные и preapply approval-хвосты закрыты; новые отзывы,
  появившиеся после approval, сохранены отдельно и не влияют на Messenger.
...
## Recovery 2026-07-27 после восстановления cookies

- exact approved tail package:
  `ozon_inbox_verify_20260727T2009_ozon_messenger_approved_20260727T201310`;
- первая recovery-попытка не отправила сообщение: textarea была найдена, но
  клик перехватило окно Ozon `Загрузите документы на бренд`;
- `ozon_send_messages_cdp.js` доработан: без owner approval он может только
  локально скрыть это точное известное окно на текущей странице, не нажимая
  кнопки и не изменяя настройку аккаунта;
- повторная попытка
  `ozon_messenger_tail_recovery_20260727T205814` отправила `1/1`;
- fresh verify `ozon_inbox_verify_cookie_recovery_20260727T2059` подтвердил:
  последнее сообщение в целевом чате от `Seller`, текст точно совпадает с
  approved reply, непрочитанных Customer-сообщений `0`, Messenger actions `0`;
- владелец отдельно подтвердил `Больше не показывать`; кнопка нажата в контуре
  Vital Shevron, новая dashboard-страница подтвердила отсутствие окна.

Fresh verify также обнаружил `35` новых Ozon-отзывов и `0` вопросов. Это новый
pending package, он не входит в старое согласование и требует отдельного owner
review.

## Recovery 2026-07-30 после частичного mark-read

- owner-approved пакет `ozon_inbox_20260730T1830` содержал `3` ответа
  покупателям и `8` действий `mark_chat_read`;
- основной apply отправил `3/3` ответа и выполнил `7/8` mark-read, но один
  сервисный чат вернул `403 Premium Plus`, поэтому общий статус запуска стал
  `blocked`;
- повторный полный apply не выполнялся: для единственного неподтвержденного
  действия использован точечный LK/CDP fallback в контуре Vital Shevron;
- Seller API history verify подтвердил точное совпадение всех трех
  согласованных ответов, отсутствие непрочитанных сообщений покупателей в
  этих чатах и `is_read=false` не осталось ни у одного из восьми целевых
  уведомлений;
- fresh verify после восстановления ЛК вернул `0` Messenger-действий;
- старые inbox/reviews approval-хвосты закрыты. Отдельно найденные `51`
  Ozon-review actions сохранены новым pending-пакетом и не включены в прежнее
  согласование.

Повторяемое правило: если apply частично завершился и idempotency marker уже
создан, нельзя повторять весь согласованный пакет. Нужно восстановить только
неподтвержденные действия, проверить каждое действие по live-источнику и
закрыть старый approval-хвост только после нулевого fresh verify.

С 2026-08-01 это правило автоматизировано в `inbox_workflow.py`: подтвержденные
ответы и mark-read получают action receipt, skipped/failed действия квитанцию
не получают, а следующий Job Worker запуск выполняет только неподтвержденный
хвост. Общий inbox marker и cleanup создаются только после `remaining_count=0`.

## Verify 2026-07-31: ложный агрегированный unread

- после успешного owner-approved apply отправлено `3/3` ответа на отзывы,
  отмечено просмотренными `16/16` отзывов и прочитанными `3/3` уведомления;
- `/api/review/counter` после apply подтвердил `NOT_VIEWED=0`, а каждое из
  трех уведомлений вернуло `unread_count=0`;
- fresh Messenger verify сначала проверил 300 чатов, затем полный доступный
  список из 363 чатов и все их истории: действий и непрочитанных сообщений в
  строках не найдено;
- при этом верхнеуровневый `total_unread_count` в `/v3/chat/list` остался `1`.
  Это подтвержденное расхождение агрегата с построчными данными, а не основание
  для повторного write;
- при таком расхождении увеличить обход до всех доступных чатов, проверить
  `unread_count` каждой строки и unread-сообщения в `/v3/chat/history`. Если
  полный обход дает ноль действий, зафиксировать агрегат как stale/weak signal
  и не выполнять безадресный `mark-read`.
