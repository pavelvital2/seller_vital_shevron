...
## Рабочий режим по одной позиции

С 2026-06-28 владелец выбрал режим работы по одной позиции end-to-end.
После согласования карточки агент не должен откладывать внедрение на
неопределенный будущий этап. Для этой же позиции нужно сразу довести проектный
контур до следующего безопасного шага:

1. сохранить owner-approved результат в Layer 3 passport;
2. обновить master catalog/связанные индексы, если они ведутся;
3. подготовить marketplace-specific dry-run для Ozon и/или WB;
4. если требуется унификация артикула продавца, включить изменение seller SKU
   в тот же review-пакет как отдельный опасный action;
5. если товара нет на второй площадке, включить создание новой карточки в
   transfer dry-run;
6. показать владельцу HTML-пакет с точным `before -> after` и всеми
   dangerous actions по позиции;
7. если владелец после просмотра этого HTML пишет `применяй`, считать это
   полным approval для всех действий, явно показанных в HTML;
8. после apply выполнить verify и сохранить результат.

Такой режим означает быстрый переход от согласования к dry-run/apply-пакету,
но не отменяет safety-цепочку и не разрешает применять изменения из Layer 2
или из устной памяти без сохраненного пакета.

Повторно показывать владельцу те же самые title, description, seller SKU,
создание карточки WB/Ozon, фото и атрибуты после команды `применяй` не нужно.
Исключения, когда агент обязан остановиться до apply:

- фактические данные в Ozon/WB изменились относительно HTML-пакета;
- API/LK требует обязательное поле, которого не было в HTML;
- действие не было явно показано в HTML;
- найден риск блокировки, модерации, правовой/контентный риск или ошибка
  mapping/seller SKU;
- apply-пакет технически отличается от HTML-пакета.
