...
Проверено 2026-08-01 для фактически выведенных денег:

- использовать строки со статусом `Выплачена` и группировать по `Дате отправки
  выплаты`, а не по планируемой дате или периоду реализации;
- сумма строки является фактически отправленной Ozon выплатой; текущий баланс,
  начисления и планируемые выплаты не прибавлять;
- `Прочие выплаты` при активном ежедневном графике содержат факторинговые
  платежи, а `Оплата выкупов маркетплейсом` учитывается отдельной строкой;
- стоимость ежедневного графика уже списывается финансовой операцией
  `OperationMarketplaceFlexiblePaymentSchedule` до формирования доступной
  выплаты. Повторно вычитать ее из реестра выплат нельзя;
- текущий интерфейс показывает суммы выплат в целых рублях. В отчете сохранять
  точность источника и проверять все страницы пагинации.
