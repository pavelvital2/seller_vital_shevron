...
- read-only источник для `/wb-inbox`: helper
  `scripts/notifications/wb_news_readonly.js` открывает эту страницу в
  сохраненной WB-сессии, собирает заголовки/даты/ссылки последних новостей и
  не сохраняет cookies/tokens/storage state.
- Важные новости выделяются в отчете по словам про тарифы, комиссии,
  логистику, маркировку, карточки, блокировки, претензии, штрафы, акции,
  продвижение, FBO/FBW/FBS, поставки и отгрузки.

Ограничение:

- этот маршрут не подтверждает статус `прочитано/непрочитано` и не выполняет
  mark-read; использовать только как read-only triage важных изменений WB.
