...
Для сравнения магазина между датами нельзя считать полным результат одного
`/warehouse/wb/query-positions?limit=500`: лимит применяется ко всему ответу и
может обрезать часть запросов/товаров. Подтвержденный порядок с 2026-07-18:

1. Зафиксировать полный union отслеживаемых запросов из обоих проверенных
   parser run.
2. Для каждого запроса отдельно запросить обе даты с
   `supplier_id=4516781`, `date_from=date_to` и `limit<=500`.
3. Сопоставлять пары `query + product_id`, отдельно считать `new`, `lost`,
   `improved`, `declined`, `unchanged`.
4. `/warehouse/wb/daily-changes` не имеет параметра `supplier_id`; нельзя
   брать первые 500 глобальных строк и затем локально фильтровать их как полный
   store-report. Для store movers использовать aggregate `/top-movers` с
   `supplier_id` или полный покверийный расчет.

Реализация и подтвержденный отчет:

```text
scripts/analytics/build_wb_parser_change_report.py
