...
## Семантика owner-facing отчетов

Не смешивать три разных состояния каталога:

- `internal_sku coverage` — целевой товар просмотрен, классифицирован и получил
  внутренний артикул; owner-review актуального ассортимента завершен;
- `marketplace pair coverage` — одна product-level запись содержит native ID
  Ozon и WB;
- `marketplace presence` — товар подтвержденно существует только на Ozon или
  только на WB.

Статус `confirmed` является историческим техническим статусом строк исходной
mapping-таблицы. Он не является полной метрикой связанных карточек, потому что
после apply связанные строки могут иметь статус `ozon_wb`. Статусы
`ozon_only`/`wb_only` после owner-review не означают непросмотренный mapping.
В отчетах владельцу показывать полноту `internal_sku`, число фактических пар
Ozon+WB и marketplace-only присутствие отдельно.
...
В режиме работы по одной позиции HTML-аудит/HTML-пакет должен сразу показывать
все связанные действия по этой позиции: изменение существующей карточки,
создание карточки на второй площадке, унификацию seller SKU, фото, атрибуты и
контент. Если владелец после просмотра такого HTML пишет `применяй`, это
считается полным approval для всех действий, явно показанных в HTML. Повторно
спрашивать подтверждение по тем же действиям нельзя. Остановиться перед apply
нужно только при drift, новом риске, недостающем обязательном поле или
расхождении apply-пакета с HTML.
