...
- если `Финальная скидка` равна `Текущей скидке`, прогноз должен сохранять
  фактический статус участия из свежей выгрузки WB;
- `останутся или будут участвовать` - текущие участники без изменения скидки
  плюс строки с реальным изменением скидки, которые проходят threshold;
...
Нельзя утверждать, что товар снимется с акции, если dry-run не меняет его
скидку. Если свежая выгрузка показывает статус `Да`, а расчетная скидка акции
выше порога, такую строку выводить отдельным риском: участие подтверждено
сейчас, скидка не меняется, будущее решение WB без write не доказано.
