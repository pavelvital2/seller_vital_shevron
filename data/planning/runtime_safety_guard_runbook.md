...
### Очередь согласований и отзыв подтверждения

Проверено 2026-08-02 после очистки исторического runtime-хвоста:

- рабочая очередь владельца содержит только `pending_review` и `approved`;
- `applied`, `verified`, `closed` и `rejected` являются историей и не должны
  называться открытой очередью или получать кнопку повторного apply;
- владелец может отозвать `approved`, пока Job Worker не зарезервировал его в
  `applying`; переход `approved -> rejected` выполняется атомарно и сохраняет
  отдельное audit-событие без marketplace write;
- `applying_unknown` не является пакетом к применению. Повторный apply из него
  заблокирован, а закрытие возможно только через доказательный verify или
  отдельную reconciliation-процедуру;
- legacy `applying_unknown` с невалидным approval package нельзя выдавать за
  проверенный или отклоненный. В интерфейсе он показывается отдельным
  историческим хвостом без action-кнопок.
