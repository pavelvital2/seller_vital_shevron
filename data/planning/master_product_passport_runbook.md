...
## Promotion из Layer 2 в Layer 3

Штатная promotion-команда должна переносить структурированные значения без
преобразования их в строковое представление Python. В частности,
`ozon_hashtags` из Layer 2 может быть списком и в Layer 3 должен остаться
чистым массивом тегов без квадратных скобок, кавычек и запятых как части
значений.

Если owner-approved Layer 2 содержит `future_ozon_create`, паспорт обязан:

- сохранить намерение создать карточку, целевой `offer_id` и комплект фото;
- добавить `ozon_card_create` в `safety.dangerous_actions`;
...
Fresh-аудиты могут хранить физические параметры в новом блоке
`proposed_final_card.physical`, а упаковку и marketplace-поля - в
`ozon_attributes` / `wb_attributes`. Promotion обязан поддерживать этот
контракт вместе со старым `target_physical_params` и промежуточным
`physical_parameters`. Внутри `physical` также допустим fresh-вариант:
`product_size_mm_each`, `package_size_mm`, `physical_item_count`,
`item_weight_g`, `package_weight_g`; WB-упаковка может находиться в
`wb_characteristics.dimensions_cm`. Promotion обязан нормализовать эти поля в
единый Layer 3 и проверить оба размера упаковки до записи. Если верхнеуровневое
`ozon_hashtags` содержит служебную подпись без символов `#`, например
`30 релевантных хештегов из ...`, это не набор тегов: нужно брать структурированный
...
Для WB полей `items_in_package` и `package_contents` явные значения из
`proposed_final_card.wb_attributes` являются приоритетными. Promotion не
должна заменять их общим шаблоном комплектации шеврона: это особенно критично
для петлиц, комплектов и товаров без липучки. После записи Layer 3 нужно
отдельно сверить строки `Количество предметов в упаковке` и `Комплектация`
с owner-approved HTML. Контракт подтверждён focused-тестом
`test_promote_preserves_explicit_wb_items_and_package_contents`.

Подтверждённое ограничение promotion на 2026-07-26: текущий конвертер может
не распознать размеры упаковки, если fresh-аудит хранит их только в
`package_dimensions_ozon_mm` и `package_dimensions_wb_cm`. До расширения
конвертера безопасный совместимый маршрут — продублировать уже согласованные
значения без пересчёта в алиасы `ozon_package_mm` и `wb_package_cm`, выполнить
dry-run и сверить оба значения в Layer 3. Это технические алиасы, а не повод
менять размер упаковки.
...
Если fresh-аудит уже содержит раздельные структурированные наборы, legacy
`media.target_marketplace_photo_set` нужно удалить: он не должен становиться
fallback поверх решений Ozon/WB. Строковые подписи в
`proposed_final_card.target_ozon_photo_set` или
`proposed_final_card.target_wb_photo_set` также не должны перекрывать
структурированные массивы. После promotion обязательно проверить количество
фото каждой площадки и локальные пути всех owner-approved защищённых файлов.
...
упаковки равен сумме физических изделий: для комплекта из двух шевронов
`20 г`. Поле `product_size_mm` хранит только размер одного изделия без слов
`каждый`, количества и повторного суффикса единицы измерения.
...
петлицами размером `80*30*5 мм`. Для `pack_qty`, количества физических изделий
и производственной нагрузки считать `1`; для упаковки считать как один
физический слой толщиной `10 мм`.
