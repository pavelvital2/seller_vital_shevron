...
Оставшаяся обязательная проверка: утром `2026-07-27` по семи полным дням
`20-26.07`. До нее не выполнять массовое повторное повышение или откат.
...
- применено и проверено `91/91`, drift `0`, mismatches `0`;
- ставки `108,07 -> 174,65 руб.`.

Действие:

1. До первого контроля не менять ставки повторно.
2. Утром `2026-07-25` сравнить три полных дня `22-24 июля` с согласованным
   baseline: заказы, расход, CPC, CTR, CPA, ДРР, фактическая маржа на изделие,
   остатки и parser-позиции тех же `nmID`.
3. Утром `2026-07-29` повторить итоговый контроль за семь полных дней
   `22-28 июля`.
4. Предупреждение по портфелю: расход выше `180 руб./день`; строка с 10
   кликами без заказа не повышается повторно, с 20 кликами без заказа
   выносится на снижение/остановку.
5. Любое снижение, остановку или новое повышение оформлять отдельным dry-run и
   owner approval.

Напоминания:

- `2026-07-25 09:00 MSK`:
  `vital-shevron-reminder-wb-cpc-20260725-0900.timer`;
- `2026-07-29 09:00 MSK`:
  `vital-shevron-reminder-wb-cpc-20260729-0900.timer`;
- оба user systemd timer имеют `Persistent=yes` и подтверждены в состоянии
  `active/waiting` с правильным `NextElapseUSecRealtime`;
- скрипты читают Telegram token только из внешнего `.env`, не сохраняют его и
  только отправляют уведомление без автоматического анализа или marketplace
  write.
