...
### Устойчивый массовый Ozon-сбор

Подтверждено 2026-07-21 на read-only сборе `136` запросов через контур Vital
Shevron:

- после открытия страницы выполнить технический warm-up запрос и не включать
  его в бизнес-результат; иначе первый listener может поймать первоначальную
  нефильтрованную загрузку таблицы;
- сохранять checkpoint после каждого seed;
- при закрытии страницы keeper-ом повторить только текущий seed после нового
  `ozon_cdp_guard`, проверки профиля и видимого маркера `Vital Shevron`;
- не заменять UI-trigger прямым `fetch` из страницы: без служебного контекста
  интерфейса endpoint вернул `HTTP 403 Failed to get company ID`; извлекать или
  сохранять auth headers ради обхода запрещено;
- частотность записывать только при exact normalized match; пустой ответ
  маркировать `no_data_returned`, а отсутствие exact строки в непустом top-50 -
  `not_returned_top50`. Оба статуса оставляют значение пустым и не означают
  нулевой спрос.

Проверка recovery: повторный устойчивый запуск обработал `136/136` seed без
ошибок и сохранил `1209` строк raw evidence. Marketplace write отсутствовал.
