...
Live-проверка 2026-07-18 по свежей поставке в Электросталь подтвердила:

- `statusID=4` - приемка еще не завершена;
- `quantity=318`, `acceptedQuantity=316`,
  `readyForSaleQuantity=295`, `unloadingQuantity=21`;
- сумма по `/goods` совпала с агрегатами поставки: `23` товарные строки;
- параллельный `stocks-report/wb-warehouses` показал только `9` единиц по
  складу, поэтому его нельзя использовать как единственный источник во время
  приемки и нельзя складывать с supply quantities без устранения пересечения.
