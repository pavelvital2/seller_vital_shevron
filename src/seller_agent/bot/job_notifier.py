...
def _liquidation_control_result(job: JobRecord, summary: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- кандидаты hard stop: `{_int(ozon.get('hard_stop_candidates'))}`; уже отключены по стопу: `{_int(ozon.get('hard_stop_already_inactive'))}`; актуальные ошибки minimum: `{_int(ozon.get('min_price_actionable_mismatches'))}`; устаревшие ссылки старого плана: `{_int(ozon.get('min_price_mismatches'))}`.",
            "",
            "Wildberries:",
            ...
            f"- кандидаты hard stop: `{_int(wb.get('hard_stop_candidates'))}`; уже отключены по стопу: `{_int(wb.get('hard_stop_already_inactive'))}`; второй ценовой шаг исторически завершен: `{_int(wb.get('second_price_stage_historically_completed'))}`; после других операций изменились скидки: `{_int(wb.get('second_price_stage_live_drift'))}`.",
            "",
