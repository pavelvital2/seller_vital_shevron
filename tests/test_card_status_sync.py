...
def test_sync_card_apply_status_closes_layer2_and_layer3(tmp_path: Path) -> None:
    ...
    _write_json(passport_path, {"identity": {"internal_sku": sku}, "approval": {"status": "owner_approved"}})
    _write_json(audit_path, {"identity": {"internal_sku": sku}, "owner_review": {"status": "owner_approved"}})
